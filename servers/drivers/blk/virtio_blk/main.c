...
void main(void) {
    ...
    virtio->negotiate_feature(VIRTIO_BLK_F_SEG_MAX | VIRTIO_BLK_F_BLK_SIZE);
    ...
    // base = offsetof(struct virtio_blk_config, size_max);
    // DBG("Maximum size of any single segment = ", virtio->read_device_config(base, sizeof(uint32_t)));
    base = offsetof(struct virtio_blk_config, seg_max);
    DBG("Maximum number of segments in a request = ", virtio->read_device_config(base, sizeof(uint32_t)));
    base = offsetof(struct virtio_blk_config, blk_size);
    DBG("Block size of disk = ", virtio->read_device_config(base, sizeof(uint32_t)));
}
